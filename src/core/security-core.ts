...
// Add more test cases for SecurityCore
describe('SecurityCore', () => {
  ...
  it('initializes with master key', async () => {
    await securityCore.initialize('testKey');
    expect(securityCore['key']).toBeDefined();
  });

  it('encrypts data after initialization', async () => {
    await securityCore.initialize('testKey');
    const encrypted = await securityCore.encrypt('test data');
  ...
  it('destroys sensitive data', () => {
    securityCore['key'] = 'testKey' as any;
    securityCore.destroy();
    expect(securityCore['key']).toBeNull();
  });

  it('throws error when not initialized during encryption', async () => {
    await expect(securityCore.encrypt('test')).rejects.toThrow('Security core not initialized');
  });

  it('throws error when not initialized during decryption', async () => {
  ...
  it('handles invalid JSON during decryption', async () => {
    await securityCore.initialize('testKey');
    const data = 'not a valid JSON';
    const encrypted = await securityCore.encrypt(data);
    const decrypted = await securityCore.decrypt(encrypted);
    expect(decrypted).toEqual(data);
  });
